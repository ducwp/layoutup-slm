...
class SLM_API_Listener {
    ...
    function creation_api_listener() {
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_create_new') {
            ...
            $lic_key_prefix = isset($_REQUEST['lic_prefix']) ? trim($_REQUEST['lic_prefix']) : $options['lic_prefix'];
    ...
    function activation_api_listener() {
        if (isset($_REQUEST['slm_action']) && trim($_REQUEST['slm_action']) == 'slm_activate') {
            ...
            global $wpdb;
            $product_ref = $fields['item_reference'];
            $tbl_name = SLM_TBL_LICENSE_KEYS;
            ...
            if ($retLic) {
                if($retLic->product_ref !== $product_ref){
                  $args = (array('result' => 'error', 'message' => 'Your License key is not match with item reference', 'error_code' => SLM_Error_Codes::LICENSE_BLOCKED));
                  SLM_API_Utility::output_api_response($args);
                }
